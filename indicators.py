...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['cp'] = np.where(
        ...
        np.where(hl_range == 0, 0.5, np.nan),
    )
    
    # 4. VR của phiên t phải so với đúng 20 phiên hoàn chỉnh trước đó.
    df['vol_sma20'] = df['volume'].rolling(window=20).mean().shift(1)
    df['vr'] = np.where(
        df['vol_sma20'] > 0,
    ...
    # Giữ các cột thô để tương thích với cache cũ; scoring v2 dùng vr và cp
    # riêng, không biến ngày giảm hoặc dữ liệu thiếu thành điểm 0.
    df['X_V'] = df['vr'] * (2.0 * df['cp'] - 1.0)
